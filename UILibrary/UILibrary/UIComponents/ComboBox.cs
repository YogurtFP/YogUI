...
namespace YogUI.Code.UIComponents
{
    public class ComboBox : UIComponent
    {
        ...
        public ComboBox(Vector2 position, params string[] items)
        {
            Position = position;
            drop = new DropDownList(position + new Vector2((-width - 15), height / 2));
            label = new TextDrawer(AssetManager.GetFont("Console"), "", position, Color.White, TextAlign.Center);
            drop.Hide();
